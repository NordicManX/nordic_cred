...
import {
    ...
    ChevronRight,
    Edit, // <--- NOVO ÍCONE
    Trash2 // Opcional: Se quiser excluir no futuro
} from "lucide-react";
...
export default function ContasPagarPage() {
    ...
    const [despesaEditando, setDespesaEditando] = useState<Despesa | null>(null); // <--- NOVO ESTADO
    ...
    const pagarConta = async (id: string, valor: number, descricao: string) => {
        const confirm = window.confirm(`Confirmar pagamento de R$ ${valor} referente a ${descricao}?`);
        if (!confirm) return;
    ...
    const handleEditar = (despesa: Despesa) => {
        setDespesaEditando(despesa); // Salva quem estamos editando
        setIsModalOpen(true);        // Abre o modal
    };

    // Função para abrir o modal em modo NOVO
    const handleNovo = () => {
        setDespesaEditando(null);    // Garante que está limpo
        setIsModalOpen(true);
    };
    ...
    return (
        <div className="space-y-6">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                ...
                <button
                    onClick={handleNovo} // <--- USA A NOVA FUNÇÃO
                                        ...
                                        <tr key={conta.id} className="hover:bg-red-50/30 transition-colors group">
                                            ...
                                            <td className="px-6 py-4 text-right flex items-center justify-end gap-2">
                                                ...
                                                {conta.status === 'pendente' && (
                                                    <button
                                                        onClick={() => pagarConta(conta.id, conta.valor, conta.descricao)}
            ...
            {/* O Modal agora recebe a despesa que está sendo editada */}
            <NovaDespesaModal
                isOpen={isModalOpen}
                onClose={() => setIsModalOpen(false)}
                onSuccess={fetchDespesas}
                despesaParaEditar={despesaEditando}  // <--- ESSA LINHA É A CHAVE!
            />
        </div>
    );
}