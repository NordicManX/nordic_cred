...
export function PaymentFlowModal({ 
  ...
  const [entryValue, setEntryValue] = useState<string | number>("");
  ...
  // --- CONVERSÕES SEGURAS ---
  const valEntrada = Number(entryValue) || 0;
  const valRecebido = Number(cashReceived) || 0;

  // CÁLCULOS
  const totalComDesconto = Math.max(0, total - descontoPontos);
  const valorFinanciado = Math.max(0, totalComDesconto - valEntrada);

  // --- TRAVA DE LIMITE (CORRIGIDA) ---
  // Tenta pegar limite_credito. Se for 0/null, tenta limite. Se ambos falharem, 0.
  const limiteDisponivel = clienteDoBanco 
    ? parseFloat(clienteDoBanco.limite_credito || clienteDoBanco.limite || 0)
    : 0;
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      if (cliente?.id) {
        ...
        setClienteDoBanco(null);
      }
    }
  }, [isOpen, total, cliente?.id]);
  ...
  async function fetchConfig() {
    ...
    if (data) {
        const vp = data.valor_do_ponto ? Number(String(data.valor_do_ponto).replace(',', '.')) : 0;
        const ppr = data.pontos_por_real ? Number(String(data.pontos_por_real).replace(',', '.')) : 1;
        setConfig({ valor_do_ponto: vp, pontos_por_real: ppr, senha_gerencial: data.senha_gerencial || '123456' });
    }
  }

  // BUSCA DADOS REAIS DO CLIENTE (COM SELECT *)
  async function fetchClienteReal(id: string) {
    setLoadingCliente(true);
    try {
        // Usamos select('*') para garantir que pegamos qualquer coluna de limite que existir
        ...
        if (data) setClienteDoBanco(data);
    } catch (e) {
        console.error("Erro fetch cliente:", e);
        toast.error("Erro ao verificar limite atualizado.");
    } finally {
  ...
  const handleAuthorize = () => {
    if (managerPassword === config.senha_gerencial) {
        ...
        setShowAuthInput(false);
        toast.success("Venda autorizada pelo gerente!");
    } else {
  ...
  const handleFinalize = () => {
    ...
    const pontosGanhosNaVenda = Math.floor(totalComDesconto * config.pontos_por_real);

    const dados = {
      forma_pagamento: method,
      detalhes: {
        parcelas: method === 'credito' ? creditInstallments : (method === 'crediario' ? crediarioParcelas : 1),
        valor_recebido: method === 'dinheiro' ? valRecebido : totalComDesconto,
        troco: method === 'dinheiro' ? Math.max(0, valRecebido - totalComDesconto) : 0,
        ...
        autorizado_por_gerente: limitOverride 
      }
    };
    onConfirm(dados);
  };
      ...
      <div className="bg-white rounded-xl w-full max-w-lg shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div className="bg-gray-50 px-6 py-4 border-b border-gray-100 flex justify-between items-center shrink-0">
          <div className="flex items-center gap-3">
            {step > 1 && <button onClick={handleBack} className="p-1 hover:bg-gray-200 rounded-full transition-colors"><ArrowLeft size={20} className="text-gray-600" /></button>}
            <div>
        ...
        <div className="p-6 overflow-y-auto custom-scrollbar">
          ...
          {step === 1 && clienteDoBanco && (clienteDoBanco.pontos_acumulados || 0) > 0 && (
              <div className="bg-purple-50 border border-purple-100 p-3 rounded-xl mb-6 shadow-sm relative overflow-hidden">
                ...
                <div className="flex items-center justify-between pt-1 mt-2 border-t border-purple-100">
          ...
          {step === 1 && (
            <div className="grid grid-cols-2 gap-3">
              ...
              <MethodButton icon={<Wallet size={24}/>} label="Crediário" onClick={() => handleMethodSelect('crediario')} color="text-orange-600" disabled={!cliente} />
            </div>
          )}
            ...
            <div className="space-y-6 animate-in slide-in-from-right-4">
              ...
              {method === 'crediario' && (
                <div className="space-y-4">
                  ...
                  {/* ALERTA DE LIMITE */}
                  {loadingCliente ? (
                      <div className="text-center py-4 text-gray-500 text-sm flex justify-center items-center gap-2">
                          <Loader2 className="animate-spin" size={16}/> Verificando limite...
                      </div>
                  ) : (
                      <>
                        {limiteExcedido && (
                                ...
                                <div className="flex items-start gap-3">
                                    ...
                                    <div className="flex-1">
                                        ...
                                        <p className="text-red-600 text-xs mt-1">
                                            Disponível: <b>{limiteDisponivel.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</b><br/>
                                            Necessário: <b>{valorFinanciado.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</b>
                                        </p>
                  ...
                  <div>
                    ...
                    <select value={crediarioParcelas} onChange={e => setCrediarioParcelas(Number(e.target.value))} className="w-full p-3 border border-gray-300 rounded-lg bg-white focus:ring-2 focus:ring-orange-500 outline-none">
                      {[1,2,3,4,5,6,10,12].map(n => <option key={n} value={n}>{n}x de {(valorFinanciado / n).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</option>)}
                    </select>
                  </div>
                  <div>
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input 
                        type="number" 
                        autoFocus 
                        className="w-full pl-10 pr-4 py-3 border border-gray-300 rounded-lg text-lg font-bold outline-none" 
                        value={cashReceived} 
                  ...
                  <div className="mt-4 p-4 bg-gray-50 rounded-lg flex justify-between items-center">
                    ...
                    <span className={`text-xl font-bold ${valRecebido >= totalComDesconto ? 'text-green-600' : 'text-red-500'}`}>{(valRecebido - totalComDesconto).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</span>
                  </div>
                </div>
              )}
          ...
          {step === 3 && (
            <div className="space-y-6 text-center animate-in zoom-in-95">
              ...
              <div>
                ...
                {valEntrada > 0 && <p className="text-xs text-orange-600 mt-1 font-bold">Entrada de {valEntrada.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })} abatida</p>}
              </div>
              ...
              <div className="bg-gray-50 rounded-lg p-4 text-sm space-y-2 text-left">
                ...
                {valEntrada > 0 && <div className="flex justify-between text-orange-600 border-t border-gray-200 pt-2 mt-2"><span>Entrada</span><span className="font-bold">- {valEntrada.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</span></div>}
        ...
        <div className="p-4 bg-gray-50 border-t border-gray-100 flex gap-3 shrink-0">
          {step === 2 ? (
             <button onClick={handleNextStep} disabled={(method === 'dinheiro' && valRecebido < totalComDesconto)} className={`w-full py-3 text-white rounded-lg font-bold flex items-center justify-center gap-2 transition-all ${limiteExcedido ? 'bg-red-500' : 'bg-blue-600 hover:bg-blue-700'}`}>
                {limiteExcedido ? "Ver Detalhes do Limite" : <>Continuar <ChevronRight size={18}/></>}
             </button>
          ) : step === 3 ? (
             <button onClick={handleFinalize} disabled={loading || (limiteExcedido && !limitOverride)} className={`w-full py-3 text-white rounded-lg font-bold flex items-center justify-center gap-2 shadow-lg transition-all ${limiteExcedido ? 'bg-gray-400 cursor-not-allowed' : 'bg-green-600 hover:bg-green-700'}`}>
